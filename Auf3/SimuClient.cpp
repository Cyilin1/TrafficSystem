...
bool bStreetOk(const string &sName, bool bNeu) {
  ...
  if (iPos > 0) {
    AfxMessageBox(CString("Wegname darf nur Buchstaben, Ziffern, -_ und keine "
                          "Blanks enthalten."));
    return false;
  }
  if (bNeu) {
...
void executeProgram(std::string programName) { system(programName.c_str()); }
...
SIMUCLIENT_API bool bInitialisiereGrafik(int sizeX, int sizeY,
  ...
  string programName = "java -jar ../../Auf3/SimuServer.jar " + Port;
  ...
  if (iCount < 0) {
    std::cout << "Error sending messageCreate: " << std::endl;

    AfxMessageBox(CString("Connect nicht moeglich! (Firewall?) Grafikausgaben "
                          "werden unterdrueckt."),
                  false);
    return false;
  }
...
SIMUCLIENT_API bool bLoescheFahrzeug(const std::string &name) { return true; }
...
SIMUCLIENT_API bool bLoescheFahrzeug(const char *name) { return true; }
...
bool x_bSet(char cTyp, double rel_position, double speed, double tank = 0.0) {
  ...
  int iPos = sFzName.find_first_not_of(
      "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890_-");
  if (iPos > 0) {
    AfxMessageBox(CString("Fahrzeugname darf nur Buchstaben, Ziffern, -_ und "
  ...
  if (tank > 999.9) {
    AfxMessageBox(
        CString("Versteckte Tanks nicht erlaubt. Tankinhalt nur < 1000 l"));
    return false;
  }
  ...
  if ((rel_position >= 0.0) && (rel_position <= 1.0)) {
    if (bStreetOk(sSName, false)) {
      if (cTyp == 'c') {
        msg << "sc " << sFzName << " " << sSName << setiosflags(ios::fixed)
            << " " << setw(7) << setprecision(4) << rel_position << " "
            << setw(6) << setprecision(1) << speed << " " << setw(6)
            << setprecision(1) << tank << "#";
      } else if (cTyp == 'b') {
        msg << "sb " << sFzName << " " << sSName << setiosflags(ios::fixed)
            << " " << setw(7) << setprecision(4) << rel_position << " "
...
SIMUCLIENT_API bool bZeichneStrasse(const char *way_to_name,
                                    const char *way_back_name, int length,
                                    int numPoints, int *points_xy) {
...
SIMUCLIENT_API bool bZeichneStrasse(const char *way_to_name,
                                    const string &way_back_name, int length,
                                    int numPoints, int *points_xy) {
  sHName = string(way_to_name);
...
SIMUCLIENT_API bool bZeichnePKW(const string &carname, const char *streetname,
                                double rel_position, double speed,
                                double tank) {
  sFzName = carname;
...
SIMUCLIENT_API bool bZeichneFahrrad(const string &bikename,
                                    const char *streetname, double rel_position,
                                    double speed) {
  sFzName = bikename;
  sSName = string(streetname);
  return x_bSet('b', rel_position, speed);
